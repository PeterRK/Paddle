...
if [[ "${GNUC_VERSION_MSG}" != '' ]]; then
	if [[ ${GNUC_VERSION_MSG:17} -gt 5 ]]; then
		CXXFLAGS="${CXXFLAGS} -Wno-error=implicit-fallthrough -Wno-error=format-truncation -Wno-error=ignored-attributes -Wno-error=cast-function-type -Wno-error=int-in-bool-context -Wno-error=ignored-qualifiers -Wno-error=aligned-new="
	fi
fi
